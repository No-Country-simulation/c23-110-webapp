...
export default function Aside() {
  const { state } = useAuth();
  const { asideOpen, setAsideOpen } = useAuth();
  ...
  useEffect(()=>{
    if (!asideOpen){
      setTimeout(() => document.getElementById('aside')!.style.display = 'none', 120)
      document.getElementById('aside')!.style.animation = 'asideDisappears 0.2s forwards'
    }else {
      document.getElementById('aside')!.style.display = 'block' 
      document.getElementById('aside')!.style.animation = 'asideAppears 0.2s forwards'
    }
  } ,[asideOpen])
  ...
  return (
    ...
    <div className="absolute left-4 top-4 z-40">
        ...
        Icon={Bars3Icon}
        w="7"
        func={() => setAsideOpen(!asideOpen)} 
      />
    </div>
    <aside
      className={`bg-primary h-full w-[260px] overflow-y-auto flex flex-col p-4 transition-all duration-300`}
      id="aside"
    >
      <nav className="flex flex-col">
        <section className="flex justify-end items-center mb-4" style={{ position: `${asideOpen ? 'relative':'inherit'}` }}>
          <div className="flex">
